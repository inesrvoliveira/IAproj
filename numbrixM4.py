...
class Board:
    ...
    def verify_adj_number(self, row,col, number):
        ...
        count=0
        if (adj_horiz[0] == number-1):
            count=1
        if(adj_horiz[0] == number+1):
            count=2
        if(adj_horiz[1] == number-1):
            count=3
        if(adj_horiz[1] == number+1):
            count=4
        if (adj_vert[0] == number-1):
            count=5
        if(adj_vert[0] == number+1):
            count=6
        if(adj_vert[1] == number-1):
            count=7
        if(adj_vert[1] == number+1):
            count=8
        return count
            ...
            for j in range(self.N):
                ...
                #for the last positions that are like islands
                    ...
                    for g in range(len(self.numbers_list)):
                        ...
                        if(adj_vert[0] != None):
                            if(self.numbers_list[g] == (adj_vert[0] + 1) or self.numbers_list[g] == (adj_vert[0] - 1)):
                                actions.append((i,j,self.numbers_list[g]))
                        if(adj_vert[1] != None):
                            if(self.numbers_list[g] == (adj_vert[1] + 1) or self.numbers_list[g] == (adj_vert[1] - 1)):
                                actions.append((i,j,self.numbers_list[g]))
                        if(adj_horiz[0] != None):
                            if(self.numbers_list[g] == (adj_horiz[0] + 1) or self.numbers_list[g] == (adj_horiz[0] - 1)):
                                actions.append((i,j,self.numbers_list[g]))
                        if(adj_horiz[1] != None): 
                            if(self.numbers_list[g] == (adj_horiz[1] + 1) or self.numbers_list[g] == (adj_horiz[1] - 1)):
                                actions.append((i,j,self.numbers_list[g]))
                    print(self.to_string()) 
